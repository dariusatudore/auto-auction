...
class coupe: public car {
  ...
  coupe(const coupe & C): car(C.brand, C.model) {}~coupe() override =
    default;
...
class supersport: public coupe {
  ...
  supersport(const supersport & S): coupe(S.brand, S.model), pret(S.pret) {}~supersport() override =
    default;
...
class cabrio: public coupe {
  ...
  cabrio(const cabrio & C): coupe(C.brand, C.model) {}~cabrio() override =
    default;
...
class hothatch: public car {
  ...
  hothatch(const hothatch & C): car(C.brand, C.model) {}~hothatch() override =
    default;
...
template < >
  class expo < supersport > {
    ...
    nrSold;
    vector < pair < supersport * ,
    int > > sold,
    exposed;
    public: expo(): inStock(0),
    nrSold(0) {}~expo() {
      for (auto m: exposed) {
        delete(m.first);
      }
